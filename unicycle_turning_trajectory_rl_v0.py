...
class UnicyclePositionLogger(BaseCallback):
    ...
    def _on_step(self) -> bool:
        ...
        self.unicycle_roll_pitch_yaw.append(euler)
...
def plot_unicycle_position(logger, run_name, iteration):
    ...
    # Unicycle roll, pitch and yaw
    ax = fig.add_subplot(223)
    unicycle_roll_pitch_yaw = np.array(logger.unicycle_roll_pitch_yaw)
    ax.plot(logger.timesteps, unicycle_roll_pitch_yaw)
    ax.set_title('Unicycle Roll, Pitch and Yaw')
...
def train(env, sb3_algo):
    ...
    eval_env = RewardWrapper(gym.make('UnicycleTurningTrajectory-v0', render_mode=None, max_episode_steps=MAX_EPISODE_STEPS))
...
if __name__ == '__main__':
    ...
    gymenv = RewardWrapper(gym.make('UnicycleTurningTrajectory-v0', render_mode=None, max_episode_steps=MAX_EPISODE_STEPS))
    ...
    if args.test:
        if os.path.isfile(args.test):
            ...
            test_env = RewardWrapper(gym.make('UnicycleTurningTrajectory-v0', render_mode='human', max_episode_steps=MAX_EPISODE_STEPS))
