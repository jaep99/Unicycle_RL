...
class UnicycleBalance(MujocoEnv, utils.EzPickle):
    """
    ## Description
    This environment simulates a unicycle. The goal is to balance the unicycle and control its movement.
    ...
    1. The unicycle falls over (z-position below a certain threshold)
    ...
    def step(self, action):
        ...
        quat_mujoco = observation[3:7]  # MuJoCo quaternion (wxyz)
        ...
        tilt_penalty = -10.0 if abs(unicycle_roll) > np.pi/4 or abs(unicycle_pitch) > np.pi/4 else 0.0
        
        # Penalize excessive wheel speed
        wheel_speed_penalty = -0.1 * observation[14]**2  # Last element is wheel angular velocity
        
        # Compute total reward
        reward = balance_reward + tilt_penalty + wheel_speed_penalty
        
        # Check termination conditions
        terminated = bool(
            abs(unicycle_roll) > np.pi/3 or  # Unicycle has fallen sideways
            abs(unicycle_pitch) > np.pi/3    # Unicycle has fallen forward/backward
        )
