...
class UnicycleTrajectory(MujocoEnv, utils.EzPickle):
    """
    ## Description
    This environment simulates a unicycle. The goal is to balance the unicycle and move it to x-coordinate 12.
    ...
    ## Rewards
    The reward function encourages balancing the unicycle and moving towards the goal position.
    ...
    2. The unicycle reaches the goal position (x-coordinate 12)
    3. The maximum number of steps is reached

    ## Solved Requirement
    The environment is considered solved when the agent can keep the unicycle balanced and move it to the goal position.
    """
    metadata = {
        "render_modes": [
            "human",
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if xml_file is None:
            xml_file = os.path.join(os.path.dirname(__file__), "assets", "unicycle_3d.xml")
    ...
    def step(self, action):
        ...
        forward_reward = x_vel  # Directly reward forward velocity
        
        # Simplified goal reaching reward
        goal_reward = -abs(self.goal_x - x_pos) / self.goal_x  # Normalize by goal distance
        ...
        terminated = bool(
            ...
            abs(x_pos - self.goal_x) < 0.1  # Goal reached
        )
        
        truncated = bool(self.steps >= self.max_steps)
        
        if abs(x_pos - self.goal_x) < 0.1:
        ...
        info = {
            ...
            "goal": goal_reward,
            "steps": self.steps,
            "goal_reached": abs(x_pos - self.goal_x) < 0.1
        }
