...
class CurriculumLearning(BaseCallback):
    ...
    def _on_step(self) -> bool:
        if self.locals.get('dones')[0]:
            ...
            if self.episode_count % self.update_interval == 0:
                ...
                self.env.env.update_difficulty(success_rate)
                self.success_count = 0
                self.episode_count = 0
            if self.locals.get('infos')[0].get('goal_reached', False):
...
class UnicyclePendulumLogger(BaseCallback):
    ...
    def _on_step(self) -> bool:
        ...
        self.target_distances.append(info.get('target_distance', 0))
        self.max_tilts.append(info.get('max_tilt', 0))
        return True
...
def plot_unicycle_pendulum_position(logger, run_name, iteration):
    ...
    ax.plot(logger.timesteps, logger.target_distances)
    ax.set_title('Target Distance')
    ax.set_xlabel('Timesteps')
    ax.set_ylabel('Distance (m)')

    # Max tilt
    ax = fig.add_subplot(326)
    ax.plot(logger.timesteps, logger.max_tilts)
    ax.set_title('Max Allowed Tilt')
    ax.set_xlabel('Timesteps')
    ax.set_ylabel('Angle (radians)')
...
def train(env, sb3_algo):
    ...
    eval_env = RewardWrapper(gym.make('UnicyclePendulumTrajectoryCurriculum-v0', render_mode=None, max_episode_steps=MAX_EPISODE_STEPS))
    ...
    # Initialize the CurriculumLearning
    curriculum_callback = CurriculumLearning(env)
    
    # Combine callbacks
    callbacks = [eval_callback, unicycle_pendulum_logger, curriculum_callback]
...
if __name__ == '__main__':
    ...
    gymenv = RewardWrapper(gym.make('UnicyclePendulumTrajectoryCurriculum-v0', render_mode=None, max_episode_steps=MAX_EPISODE_STEPS))
    ...
    if args.test:
        if os.path.isfile(args.test):
            ...
            test_env = RewardWrapper(gym.make('UnicyclePendulumTrajectoryCurriculum-v0', render_mode='human', max_episode_steps=MAX_EPISODE_STEPS))
